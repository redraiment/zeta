(ns zeta.opt
  "Zeta框架的全局选项，用于功能插件的初始化。
  !!!非应用配置项!!!

  加载过程
  1. `zeta.` 开头的系统属性（System Property），但不需要指定`zeta`
  2. resource://zeta.edn，其中不包含`zeta`
  ...
  * 其他：返回字符串
  ...
  {:a {:b 1
       :c 2}}
  ```
  2、启动参数包含`-Dzeta.a.b=3`
  则`(zeta/of [:a :b])`返回整数`3`"
...
(def ^:dynamic *options*
  "配置文件选项"
  (delay (edn-of "resource://zeta.edn")))
...
(defmacro ^:private defn-option
  ...
  `(defn-default ~name ~comment [ks#]
     (let [~'key-list (if (coll? ks#) ks# [ks#])
           ~'property-name (to-property-name (into [:zeta] ~'key-list))]
       ~@body)))

(defn-option of
  "返回当前上下文的选项值
  1. 先从 System.Properties 中获取 zeta. 开头的值
  2. 再从 zeta.edn 中获取
  返回单个值"
